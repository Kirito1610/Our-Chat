...
export async function requireAdmin(conversationId, userId) {
    ...
    if (member.role !== "admin")
        throw new AppError(403, "ADMIN_REQUIRED", "Group admin permission required");
    return member;
}
export async function conversationDetails(conversationId, userId) {
...
export async function createConversation(userId, input) {
    if (input.type === "direct") {
        if (input.participantId === userId)
            throw new AppError(400, "SELF_CONVERSATION", "Cannot create a conversation with yourself");
    ...
    if (existingUsers.length !== memberIds.length)
        throw new AppError(400, "INVALID_MEMBERS", "One or more members do not exist");
    ...
    if (!created)
        throw new AppError(500, "CREATE_FAILED", "Could not create conversation");
    await db.insert(conversationMembers).values(memberIds.map((memberId) => ({ conversationId: created.id, userId: memberId, role: memberId === userId ? "admin" : "member" })));
    return conversationDetails(created.id, userId);
}
export async function listConversations(userId) {
    ...
    return Promise.all(rows.map(async ({ id }) => {
        ...
        const [unread] = await db.select({ count: sql `count(*)::int` }).from(messages).where(and(eq(messages.conversationId, id), me?.lastReadMessageId ? sql `${messages.createdAt} > coalesce((select created_at from messages where id = ${me.lastReadMessageId}), 'epoch')` : sql `true`, sql `${messages.senderId} is distinct from ${userId}`));
