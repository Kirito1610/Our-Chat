...
import { conversationMembers, conversations, messageReactions, messages, users } from "../../db/schema.js";
import { AppError } from "../../lib/errors.js";
import { decodeCursor, encodeCursor } from "../../lib/pagination.js";
import { realtimeHub } from "../../realtime/hub.js";
import { requireMember } from "../conversations/conversation.service.js";
...
export async function sendMessage(conversationId, userId, input) {
    ...
    if (input.replyToId) {
        ...
        if (!reply || reply.conversationId !== conversationId)
            throw new AppError(400, "INVALID_REPLY", "Reply target is not in this conversation");
    }
    if (input.clientId) {
        ...
        if (existing)
            return getMessage(existing.id, userId);
    }
    const [created] = await db.insert(messages).values({ conversationId, senderId: userId, ...input, attachments: input.attachments }).returning({ id: messages.id });
    if (!created)
        throw new AppError(500, "SEND_FAILED", "Could not send message");
...
export async function listMessages(conversationId, userId, rawCursor, limit) {
    await requireMember(conversationId, userId);
...
export async function editMessage(messageId, userId, content) {
    ...
    if (message.senderId !== userId)
        throw new AppError(403, "NOT_MESSAGE_OWNER", "You can only edit your own messages");
    if (message.deletedAt)
        throw new AppError(400, "MESSAGE_DELETED", "Deleted messages cannot be edited");
...
export async function deleteMessage(messageId, userId) {
    const message = await getMessage(messageId, userId);
    if (message.senderId !== userId)
        throw new AppError(403, "NOT_MESSAGE_OWNER", "You can only delete your own messages");
...
export async function setReaction(messageId, userId, emoji, remove = false) {
    const message = await getMessage(messageId, userId);
    if (remove)
...
export async function markRead(conversationId, userId, messageId) {
    ...
    if (!message)
        throw new AppError(400, "INVALID_MESSAGE", "Message is not in this conversation");
